...
<button class="btn default c-btn-square p-l-10 p-r-10" type="button"><i class="fa fa-calendar"></i></button>
                        ...
                        <input type="text" class="form-control c-square c-theme" name="started_at" autocomplete="off"
                               placeholder="Từ ngày" value="<?= $_POST['started_at']; ?>">
                    </div>
                </div>
            </div>
            <div class="col-md-4">
...
<button class="btn default c-btn-square p-l-10 p-r-10" type="button"><i class="fa fa-calendar"></i></button>
                        ...
                        <input type="text" class="form-control c-square c-theme" name="ended_at" autocomplete="off"
                               placeholder="Đến ngày" value="<?= $_POST['ended_at']; ?>">
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-md-4">
                ...
                <a class="btn c-btn-square m-b-10 btn-danger" href="/profile/tran_acc">Tất cả</a>
            </div>
        </div>
    </form>
    <table class="table table-hover table-custom-res">
        ...
        if (isset($_POST['submit'])) {
            ...
            $data = $db->query("SELECT * FROM `TMQ_history` WHERE `buyer` = '" . TMQ_user() ['username'] . "' AND `loai` = 'muanick' $started $ended LIMIT $from,$limit");
        } else {
            $data = $db->query("SELECT * FROM `TMQ_history` WHERE `buyer` = '" . TMQ_user() ['username'] . "' AND `loai` = 'muanick' LIMIT $from,$limit");
        }
        foreach ($data as $row) {
    ...
    <div class="data_paginate paging_bootstrap paginations_custom" style="text-align: center">
        <?php
        $tong = $db->query("SELECT `id` FROM `TMQ_history` WHERE `buyer` = '" . TMQ_user() ['username'] . "' AND `loai` = 'muanick'")
            ->rowcount();
        if ($tong > $limit) {
            echo '<center>' . TMQ_phantrang('/profile/tran_acc?', $from, $tong, $limit) . '</center>';
        } ?>
    </div>
</div>
</div>
