...
<select name="search_<?=TMQ_xoadau($filter[0]);?>" class="form-control c-square" title="-- Không chọn --">
...
<div class="input-group c-square">
<span class="input-group-addon"><?=$filter[0];?></span>
<input name="search_<?=TMQ_xoadau($filter[0]);?>" type="text" class="form-control c-square" value="" placeholder="<?=$filter[0];?>">
</div>
</div>
<?php }} ?>
...
if(isset($_POST['submit'])){
    ...
    if($search_noibat){ $search_noibat = "AND `thongtin` LIKE '%$search_noibat%'"; }
    //tìm kiếm theo id
    $search_id = (int)$_POST['id'];
    if($search_id){  $search_id = "AND `id` LIKE '%$search_id%'"; }else{ $search_id = ""; }
    ...
    if($post){$search_filter = "AND `search` LIKE '%$post%'";}
