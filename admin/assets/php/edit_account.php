...
if(!$id || $data['id'] == null || $data['username'] != TMQ_user()['username'] || $data['trangthai'] == 'off'){ header("Location: /admin/account"); }
...
$chuyenmuc = $chuyenmuc['name'];
  ...
  if(isset($_POST['edit']))
  {
     ...
     if(empty($taikhoan) || empty($matkhau) || empty($cash) || empty($thongtin) || empty($image) || empty($res)){
                    ...
                    <h4 class="card-title">Sửa tài khoản #<?=$id;?></h4>
                      <form class="forms-sample" method="POST">
                       <div class="row form-group">
                        ...
                        <input type="text" class="form-control" name="matkhau" placeholder="Nhập mật khẩu" value="<?=$data['matkhau'];?>">
                      </div>
                       <div class="col-3">
 ...
 if($filter[1] == 'select'){ ?>
     ...
     if($filter[1] == 'input'){ ?>
      <div class="col-3">
                       ...
                       <input type="text" class="form-control" name="<?=TMQ_xoadau($filter[0]);?>" placeholder="Nhập <?=$filter[0];?>..." value="<?=$kq[1];?>">
               </div> 
<?php } 
if($e == $dem){ echo "</div>"; } 
 }
