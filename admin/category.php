...
if(isset($_POST['del']))
{
    ...
    if(isset($_POST['cancel'])){ header('Location: /admin/category'); }
echo '<div class="row">
...
if(isset($_POST['edit'])){
    $get = $db->query("SELECT `name`,`id_cmm` FROM `TMQ_chuyenmuc` WHERE `id` = '".TMQ_check($_POST['edit'])."'")->fetch();
    if(isset($_POST['confirm'])){
        $name = TMQ_check($_POST['name']);
        $id = (int)$_POST['edit'];
        $back_url = TMQ_check($_POST['back_url']);
        $db->exec("UPDATE `TMQ_chuyenmuc` SET `name` = ".$db->quote($name)." WHERE `id` = '$id'");
                ...
                <div class="card">
                  <div class="card-body">
                    ...
                    <form class="forms-sample" method="POST">
                        ...
                        <input type="text" class="form-control" placeholder="Category name" name="name" value="<?=$get['name'];?>">
                      </div>
                       <div class="form-group">
                      <label>Loại</label>
                      <select class="js-example-basic-single" style="width:100%" name="species">
                        <option value="0" <?php if($subcategory == 0){ echo 'selected';}?>>Chuyên mục mẹ</option>
                        <?php $get_cm = $db->query("SELECT `id`,`name`,`id_cmm` FROM `TMQ_chuyenmuc` WHERE `loai` = '0' AND `id_cmm` = '0' AND `status` = 'on'"); 
                        foreach($get_cm as $r){
                        ?>
                       <option value="<?=$r['id_cmm'];?>"<?php if($get['id_cmm'] == $r['id']){echo 'selected';} ?>><?=$r['name'];?></option>
                        <?php } ?>
                      </select>
                    </div>
...
if(isset($_GET['add'])){
    if(isset($_POST['add'])){
        ...
        if(empty($img)){ $img = "https://demo9.tmquang.monster/images/no_image.png";}
        if($species == 0){ $loai = 0; }else{ $loai = 1; }
        if(!empty($name)){
           $db->exec("INSERT INTO `TMQ_chuyenmuc`
            (`name`, `image`, `loai`, `id_cmm`, `status`, `date`)
            VALUES
            (".$db->quote($name).",".$db->quote($img).",".$db->quote($loai).",".$db->quote($species).",'on','".date("d-m-Y")."')
            ");
            header('Location: /admin/category');
        }
    }
                  ...
                  <div class="card-body">
                    ...
                    <form class="forms-sample" method="POST">
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Image name" name="image">
                      </div>
                      <div class="form-group">
                      <label>Loại</label>
                      <select class="js-example-basic-single" style="width:100%" name="species">
...
?>
                          <tr>
                            ...
                            <td class="dropdown">
                            <form method="POST"> 
                            <p><button type="submit" name="edit" value="<?=$row['id'];?>" class="btn btn-outline-secondary btn-icon-text">
...
?>
                          <tr>
                            ...
                            <td><?=$row['name'];?></td>
                            <td>
                                <p><label class="badge badge-success">Đang bán: <?=$on;?></label></p>
                                <p><label class="badge badge-danger">Đã bán: <?=$off;?></label></p>
                            </td>
                            <td><?=$status[$row['status']];?></td>
                            <td><?=$row['date'];?></td>
                            <td>
