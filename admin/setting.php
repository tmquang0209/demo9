...
if(isset($_POST['setting_themes'])){
   ...
   if($pos === false){ echo('<script>alert(\'Xóa bản quyền làm gì?\');</script>'); }else{
...
if(isset($_POST['setting_api'])){
   ...
   $db->exec("UPDATE `TMQ_setting` SET `api_napthe` = '$id\n$partner'"); 
...
if(isset($_GET['infomation'])): ?>
 <form class="forms-sample" method="POST">
                        ...
                        <label>Title</label>
                        <input type="text" class="form-control" placeholder="Title Website" name="title" value="<?=TMQ_setting()['title'];?>">
                      </div>
                      <div class="form-group">
                        <label>Logo</label>
                        <input type="text" class="form-control" placeholder="Logo url" name="logo" value="<?=TMQ_setting()['logo'];?>">
                      </div>
                     
                     <div class="form-group">
                        <label>Email</label>
                        <input type="text" class="form-control" placeholder="email" name="email" value="<?=TMQ_setting()['email'];?>">
                      </div>
                     
                     <div class="form-group">
                        <label>Number phone</label>
                        <input type="text" class="form-control" placeholder="Number phone" name="phone" value="<?=TMQ_setting()['phone'];?>">
                      </div>
                     
                     <div class="form-group">
                        <label>Facebook</label>
                        <input type="text" class="form-control" placeholder="Url fanpage" name="facebook" value="<?=TMQ_setting()['facebook'];?>">
                      </div>
                     
                     <div class="form-group">
                        <label>Youtube</label>
                        <input type="text" class="form-control" placeholder="Url youtube" name="youtube" value="<?=TMQ_setting()['youtube'];?>">
                      </div>
                     
                     <div class="form-group">
                        <label>1 page per page limit</label>
                        <input type="text" class="form-control" placeholder="1 page per page limit" name="limit" value="<?=TMQ_setting()['limit_page'];?>">
                      </div>
                      <div class="form-group">
                        <label>Notification</label>
                        <textarea rows="10" type="text" class="form-control" placeholder="Notification" name="notification"><?=TMQ_setting()['thongbao'];?></textarea>
                      </div>
                      <div class="form-group">
                        <label>Maintenance</label>
                        <select class="form-control" name="baotri">
                                            <option <?php if(TMQ_setting()['baotri'] == 'on'){ ?>selected<?php } ?>  value="on">Hoạt động</option>
                                            <option <?php if(TMQ_setting()['baotri'] == 'off'){ ?>selected<?php } ?>  value="off">Bảo trì</option>
                                        </select>
                                        </div>
...
?>
     <form class="forms-sample" method="POST">
                        ...
                        <input type="text" class="form-control" placeholder="Username..." name="username" value="<?=$email[0];?>">
                      </div>
                      <div class="form-group">
                        <label>Password</label>
                        <input type="text" class="form-control" placeholder="Password..." name="password" value="<?=$email[1];?>">
                      </div>
                       <div class="form-group">
                        <label>Port</label>
                        <input type="number" class="form-control" placeholder="Port..." name="port" value="<?=$email[2];?>">
                      </div>
...
if(isset($_GET['setting_api'])):
    $napthe = explode(PHP_EOL,TMQ_setting()['api_napthe']);
...
<input type="text" class="form-control" name="id" value="<?=$napthe[0];?>" />
    </div>
    <div class="form-group"><label>Partner Key</label>
<input type="text" class="form-control" name="partner_key" value="<?=$napthe[1];?>" />
    </div>
    <div style="color:red;">
        <p style="font-size:20px;">
        Note: nếu chưa có tài khoản napthenhanh.com thì liên hệ: <a href="https://www.facebook.com/hoangminhquan.wikvnn/" target="_blank">Hoàng Minh Quân</a> để được cấp tài khoản
        </p>
        <p style="font-size:20px;">
            Link callback: <?='http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . "{$_SERVER['HTTP_HOST']}/profile/callback";?>
        </p>
        </div>
    <hr style="border-top: 1px dashed yellow;">
...
<input type="text" class="form-control" name="app_secret" value="<?=$login[1];?>" />
    ...
    <div style="color:red;">
        <p style="font-size:20px;">
        Note: Truy cập vào <a href="http://developer.facebook.com/" target="_blank">Developer.facebook.com</a> để tạo app
        </p>
        <p style="font-size:20px;">
           Miền ứng dụng nhập: <?=TMQ_domain();?>
        </p>
        <p style="font-size:20px;">
           Url chuyển hướng hợp lệ: <?='http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . "{$_SERVER['HTTP_HOST']}/login_facebook";?> và <?='http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . "{$_SERVER['HTTP_HOST']}";?>
        </p>
        </div>
